...
* Imagine a 3D tensor (batch, seq_len, emb_dim).
  For batch norm, for each feature index $j$ (across all tokens and examples in the batch), compute:
  $$\mu_\text{batch}^{(j)} = \text{mean of feature } j \text{ across batch}$$
  $$\sigma_\text{batch}^{(j)} = \text{std of feature } j \text{ across batch}$$
  Then:
  $$\text{Norm}*\text{batch}(x^{(j)}) = \gamma^{(j)} \cdot \frac{x^{(j)} - \mu*\text{batch}^{(j)}}{\sigma_\text{batch}^{(j)} + \epsilon} + \beta^{(j)}$$
...
In practice, the terms *normalization* and *standardization* are sometimes used interchangeably (e.g. in Keras).
...
[3] G. E. Hinton *et al.*, “Improving Neural Networks by Preventing Co-Adaptation of Feature Detectors,” arXiv:1207.0580, 2012.

[4] N. Srivastava *et al.*, “Dropout: A Simple Way to Prevent Neural Networks from Overfitting,” *J. Mach. Learn. Res.*, vol. 15, pp. 1929–1958, 2014.

[5] M. Purandare, *Transformer Architecture – A Visual Guide*, Lecture Notes, Generative AI, OST – Eastern Switzerland University of Applied Sciences, Fall 2024. 
